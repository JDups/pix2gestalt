...
class ImageLogger(Callback):
    ...
    def log_local(self, save_dir, split, images,
        ...
        subroot_name = "gs-{:06}_e-{:06}_b-{:06}".format(
                global_step,
                current_epoch,
                batch_idx)
        subroot = os.path.join(root, subroot_name)
        os.makedirs(subroot, exist_ok=True)

        # save images individually
        for k in images:
            k_path = os.path.join(subroot, k)
            os.makedirs(k_path, exist_ok=True)
            base_count = len(glob.glob(os.path.join(k_path, "*.png")))
            for img in images[k]:
                if self.rescale:
        ...
        gt_target_wholes = images["inputs"]
        conditioning_occlusions = images["conditioning"]
        scale1_preds = images["samples"]
        scale3_preds = images["samples_cfg_scale_3.00"]
        
        visualization_dir = os.path.join(subroot, "visualizations")
        os.makedirs(visualization_dir, exist_ok=True)
        fullgrid_images = list()
        for i in range(len(conditioning_occlusions)):
            target_image = gt_target_wholes[i]
            conditioning_image = conditioning_occlusions[i]
            scale1_pred = scale1_preds[i]
            scale3_pred = scale3_preds[i]
            
            fullgrid_images.extend([conditioning_image, target_image, scale1_pred, scale3_pred])

            # current sample plot
            grid = torchvision.utils.make_grid([conditioning_image, target_image, scale1_pred, scale3_pred])
            if self.rescale:
                grid = (grid + 1.0) / 2.0  # -1,1 -> 0,1; c,h,w
        ...
        half1_images = fullgrid_images[:len(fullgrid_images)//2]
        half2_images = fullgrid_images[len(fullgrid_images)//2:]

        grid = torchvision.utils.make_grid(half1_images, nrow=4)
        if self.rescale:
            grid = (grid + 1.0) / 2.0  # -1,1 -> 0,1; c,h,w
        grid = grid.transpose(0, 1).transpose(1, 2).squeeze(-1)
        grid = grid.numpy()
        ...
        fullgrid_filename = "all1_gs-{:06}_e-{:06}_b-{:06}.png".format(
                global_step,
                current_epoch,
                batch_idx)
        fullgrid_path = os.path.join(visualization_dir, fullgrid_filename)
        Image.fromarray(grid).save(fullgrid_path)

        grid = torchvision.utils.make_grid(half2_images, nrow=4)
        if self.rescale:
            grid = (grid + 1.0) / 2.0  # -1,1 -> 0,1; c,h,w
        grid = grid.transpose(0, 1).transpose(1, 2).squeeze(-1)
        grid = grid.numpy()
        ...
        fullgrid_filename = "all2_gs-{:06}_e-{:06}_b-{:06}.png".format(
                global_step,
                current_epoch,
                batch_idx)
        fullgrid_path = os.path.join(visualization_dir, fullgrid_filename)
    ...
    @rank_zero_only
    def log_local_grid(self, save_dir, split, images,
                  global_step, current_epoch, batch_idx):
        root = os.path.join(save_dir, "images", split)
        for k in images:
...
if __name__ == "__main__":
    ...
    try:
        ...
        if not opt.finetune_from == "":
            ...
            """
            Comment this block for ablation training, uncomment for regular fine-tuning.
            """ 
        ...
        if hasattr(model, "monitor"):
            ...
            default_modelckpt_cfg["params"]["save_top_k"] = 15
        ...
        if not cpu:
            ...
            ngpu = 1 # Changed like this for now for running on single GPU colab
        else:
            ngpu = 1
